String[] ArrowStands = new String[31];
...
void setup(){
 ...
 ArrowStands[30] = "Monochromatic Purple Haze";
 TrowelDig[0] = "Nothing";
...
public void rareRoll(){
  if((MyPrivateServer.getStand().equals("Doppio's King Crimson")) && MyPrivateServer.getFrogs() >= 2){
    ...
    if ((int)(Math.random()*100) >= 1){
       ...
       if (select[x] == "Manga" && statsMode == true){
    ...
    if ((MyPrivateServer.getSpheres() > 0) && (MyPrivateServer.getStand().equals("Full-Body King Crimson")) && !(statsMode == true && select[x].equals(ArrowStands[30]))){
      ...
      if (select[x] == "Oreo" && statsMode == true){
  ...
  if((!MyPrivateServer.getStand().equals("Whitesnake")) && (!MyPrivateServer.getStand().equals("The World")) && (MyPrivateServer.getMasks() >=2)){
     ...
     if((int)(Math.random()*17) >= 1){
       ...
       if (select[x] == "Kars" && statsMode == true){
        Statistics.setKarsSample(MyPrivateServer.getTime());
        MyPrivateServer.reset();
       }
     }
  ...
  if((MyPrivateServer.getStand().equals("Whitesnake")) && (MyPrivateServer.getDiaries() >= 2)){
     ...
     if ((int)(Math.random()*100) >= 1){
       ...
       if (select[x] == "MiG" && statsMode == true){
  ...
  if((MyPrivateServer.getStand().equals("The World")) && (MyPrivateServer.getMasks() >= 1) && (MyPrivateServer.getCams() >= 1)){
    ...
    if (select[x] == "STW" && statsMode == true){
...
public class ConfidenceInterval{
  ...
  public String interval(String s){
    ...
    if(s.equals(ArrowStands[c]) && customSample.size()>=30){
      list = new double[customSample.size()];
      for(int i=0; i<customSample.size();i++){
        add += customSample.get(i);
        list[i]=customSample.get(i);
      }
      mean = add/customSample.size();
      stDev = sdDev(list);
